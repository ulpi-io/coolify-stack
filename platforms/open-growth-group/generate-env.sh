...
grep -qx 'PLATFORM_SLUG=open-growth-group' "$shared_env" || { echo "Expected the open-growth-group shared fragment" >&2; exit 1; }
...
tmp=$(mktemp "$(dirname "$output")/.open-growth-group.env.XXXXXX")
{
  cat "$shared_env"
  cat <<EOF
