...
grep -qx 'PLATFORM_SLUG=togglebox' "$shared_env" || { echo "Expected the togglebox shared fragment" >&2; exit 1; }
...
tmp=$(mktemp "$(dirname "$output")/.togglebox.env.XXXXXX")
{
  cat "$shared_env"
  cat <<EOF
