...
grep -qx 'PLATFORM_SLUG=open-kudos' "$shared_env" || { echo "Expected the open-kudos shared fragment" >&2; exit 1; }
...
tmp=$(mktemp "$(dirname "$output")/.open-kudos.env.XXXXXX")
{
  cat "$shared_env"
  cat <<EOF
...
GOOGLE_CHAT_PROJECT_NUMBER=1023476457352
