...
grep -qx 'PLATFORM_SLUG=twenty' "$shared_env" || { echo "Expected the twenty shared fragment" >&2; exit 1; }
...
tmp=$(mktemp "$(dirname "$output")/.twenty.env.XXXXXX")
{
  cat "$shared_env"
  cat <<EOF
...
EMAIL_SYSTEM_ADDRESS=system@crm.con.fyi
EOF
} > "$tmp"
chmod 600 "$tmp"
mv -f "$tmp" "$output"
